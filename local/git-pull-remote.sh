...
if [ "${POST_PULL_SCRIPT}" ]; then
	SERVER_POST_PULL_CMD="$ENV_REPODIR/$POST_PULL_SCRIPT;";
fi


SERVER_COMMANDS="$SERVER_PULL_CMD;$SERVER_POST_PULL_CMD $ENV";

#echo $SERVER_COMMANDS;
#exit;

echo ssh $ENV_CUSTOM_SSHCONNECTIONSTR -t $SERVER_COMMANDS;
